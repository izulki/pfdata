...
export async function runStripeReconcile(
    ...
    let logid = await logToDBStart(db, "stripeReconcile", method); //Send to start log to DB, return log id.
    let errors = 0;

    const logger = createLogger({
        levels: config.syslog.levels,
        format: combine(
            label({ label: 'stripeReconcile' }),
            timestamp(),
            json()
        )
        ,
        transports: [
            new transports.File({ filename: `logs/stripeReconcile/${logid}.log` }),
        ],
    });
    ...
    try {
      ...
      logger.error(error.message);
        ...
        logToDBEnd(db, logid, "COMPLETED", errors, `/logs/stripeReconcile/${logid}.log`)
    }
}